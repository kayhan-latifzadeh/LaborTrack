...
def extract_features(data_list, class_label, target_interval_codes, chunk_size):
    ...
    for data in data_list:
        ...
        data = data[['gyro_x','gyro_y','gyro_z']]
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Extract handcrafted features from head movements data.')
...
def main():
    ...
    session_1_data = load_csv_files('output/segmented_data/head-movements/session_1')
    session_2_data = load_csv_files('output/segmented_data/head-movements/session_2')
    ...
    np.save('output/X_headmovements.npy', X)
    np.save('output/X_headmovements.npy', y)
    
    print('X: output/X_headmovements.npy')
    print('y: output/y_headmovements.npy')
