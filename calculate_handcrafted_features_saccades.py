...
def extract_features(data_list, class_label, target_interval_codes, chunk_size):
    ...
    for data in data_list:
        ...
        timestamps = data[['start']]
        data = data[['amplitude','peak_velocity','peak_acceleration']]
        ...
        for i in range(0, len(data), chunk_size):
            ...
            start = ts_chunk.iloc[0]['start']
            end = ts_chunk.iloc[-1]['start']
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Extract handcrafted features from saccade data.')
    parser.add_argument('--chunk_size', type=int, default=10, help='Size of chunks (timestep) for feature extraction (default: 10)')
...
def main():
    ...
    session_1_data = load_csv_files('output/segmented_data/saccades/session_1')
    session_2_data = load_csv_files('output/segmented_data/saccades/session_2')
    ...
    np.save('output/saccades.npy', X)
    np.save('output/saccades.npy', y)
    
    print('X: output/X_saccades.npy')
    print('y: output/y_saccades.npy')
